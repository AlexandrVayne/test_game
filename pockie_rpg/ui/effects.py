...
Stage 204 — иерархия вместо копипасты: LoopingOverlay (frame-advance цикл +
рендер по центру) и OneShotEffect (проиграть один раз и выключиться). Вид и
API всех четырёх классов не изменились.
"""
...
class LoopingOverlay:
    """Зацикленная анимация по папке кадров: ядро activate/deactivate/update/render.

    Пока active — кадры листаются по кругу (frame_period = 1/fps). Рендер —
    текущий кадр по центру (cx, cy); размер ×scale (Stage 201: позиция приходит
    уже в пространстве рендера, масштабируется только размер спрайта).
    """
...
class EffectOverlay(LoopingOverlay):
...
class IceBlockEffect(EffectOverlay):
    """Ice block animation overlay rendered on top of a frozen fighter."""

    def __init__(self) -> None:
        super().__init__(
            ICE_BLOCK_FOLDER, ICE_BLOCK_FPS, ICE_BLOCK_RENDER_W, ICE_BLOCK_RENDER_H,
        )


@dataclass
class OneShotEffect:
    """Одноразовая анимация: проигрывает папку кадров один раз и гаснет.

    Ядро deactivate/update для CastEffect и ProjectileEffect (Stage 204 —
    были дословными копиями в обоих классах).
    """
...
@dataclass
class CastEffect(OneShotEffect):
    ...
    def render(
        ...
        if not self.active or self._frame_count <= 0:
            return
        sprite = self._sprite(asset_manager, scale)
...
@dataclass
class ProjectileEffect(OneShotEffect):
    ...
    def render(
        ...
        sprite = self._sprite(asset_manager, scale, flip=self.flip)
