...
class SkillsCharSheetRendererMixin:
    ...
    def _render_char_sheet(self, close_cb=None) -> None:
        ...
        if self._char_sheet_target == "player":
            ...
            # Stage 206 — имя игрока от костюма (Абарая надет — «Ренджи Абараи»).
            _suit = resolve_player_suit(self.player)
            display_name = (_suit.name if _suit else None) or (role.name if role else "")
        else:
