...
# Stage 204 — SYNTH_SLOT_PAD удалён (сетка центрируется — отступ вычисляется).
SYNTH_SLOT_COLORS = {
    "main": (234, 179, 8),
    "cat": (100, 180, 255),
    "result": (80, 220, 100),
...
class SynthRendererMixin:
    ...
    def _draw_synth_slot_grid(self, rect: pygame.Rect) -> None:
        """Stage 184/186 — сетка 2×3 внутри слота, ЦЕНТРИРОВАНА (минимальный
        отступ ~3px от краёв слота — запрос пользователя).
