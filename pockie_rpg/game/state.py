...
class PlayerState:
    ...
    # Stage 206 — разовый грант 2× suit_cloth14 («Ренджи Абараи») старым
    # сейвам (тест смены аватара/статов/анимаций). Паттерн — как v160.
    suit14_grant_v206: bool = False
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            # Stage 206 — флаг разового гранта костюма Абарая.
            "suit14_grant_v206": self.suit14_grant_v206,
    ...
    def from_dict(cls, data: dict) -> "PlayerState":
        ...
        if data.get("synth_test_grant_v160", False):
            ...
            p.synth_test_grant_v160 = True
        # Stage 206 — разовый грант 2× suit_cloth14 («Ренджи Абараи») старым
        # сейвам: флага нет → выдаём и запоминаем (паттерн Stage 160). Новые
        # сейвы получают костюм через starter_items() — флаг у них сразу True.
        if data.get("suit14_grant_v206", False):
            p.suit14_grant_v206 = True
        else:
            p.inv_add("suit_cloth14")
            p.inv_add("suit_cloth14")
            p.suit14_grant_v206 = True
...
def create_default_player() -> PlayerState:
    ...
    p.synth_test_grant_v160 = True
    # Stage 206 — костюм Абарая уже в starter_items(); флаг блокирует
    # повторный грант при первом же from_dict (идемпотентность).
    p.suit14_grant_v206 = True
    return p
