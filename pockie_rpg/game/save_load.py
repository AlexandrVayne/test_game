...
прямая загрузка слота (F8 dev-кнопки).
...
Per Stage 9 spec:
  ...
  * Stage 204 — has_save()/delete_save() удалены (0 вызовов; сейв всегда
    автоуправляется SaveManager).
...
# Stage 204 — TEMP_FILE_PATH удалён вместе с delete_save (атомарная запись
# пишет tmp-файл инлайном в _write_save).
...
def load_from_slot(n: int) -> PlayerState | None:
    """Stage 171 — B1: прямая загрузка PlayerState из слота N (F8 dev-кнопки).
