...
class AssetManager:
    ...
    def __init__(self) -> None:
        ...
        # Stage 91 — font cache (per DEVELOPMENT_RULES §8.1).
        # Avoids creating pygame.font.SysFont() on every fallback render.
    ...
    def needs_flip_for_enemy(self, folder: str, mob_id: str = "") -> bool:
        ...
        HARD RULE (per config.ENEMY_MUST_FACE_LEFT): ALL enemies face LEFT in
    ...
    def get_font(self, name: str = "dejavusans,arial", size: int = 13, bold: bool = False) -> pygame.font.Font:
        """Return a cached ``pygame.font.SysFont``.

        Per DEVELOPMENT_RULES §8.1: avoids creating SysFont on every frame
        in fallback paths. The cache key is ``(name, size, bold)``.
        """
        key = (name, size, bold)
