...
def try_skill(
    ...
    dict (set by the F9 admin panel) contains an entry for `skill_id`, that
...
# Stage 123 — Global skill chance overrides (set by F9 admin panel).
...
def set_skill_chance_overrides(overrides: dict[int, float]) -> None:
    ...
    Called by the F9 admin panel when the user adjusts a skill chance.
    """
