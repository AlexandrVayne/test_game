...
TOWER_MODIFIERS: frozenset[str] = frozenset({
    "boss_shield_at_half_hp",     # metadata only (phase system not in v1)
    "boss_poison_resistance",     # applied: +tough_rating
    "boss_fast_start",             # applied: +action_points at battle start
    "boss_enrage_at_half_hp",     # metadata only (phase system not in v1)
    "boss_second_phase",           # metadata only (reserved)
})
...
def is_boss_floor(floor: int) -> bool:
    """Return True if ``floor`` is a boss floor (every 10th floor)."""
    return floor in TOWER_BOSS_FLOORS
